 ...
 *  Date: 27/03/19
 *  Input model for disk TW Hydrae CO 2-1 emission
 * 
 */ 

#include <stdio.h>
#include "lime.h"
#include "structureTW.h"
...
input(inputPars *par, image *img){
  ...
  par->sampling	= 0;
  par->outputfile = "populationsTW.pop";
  par->gridfile	= "gridTW.vtk";
  ...
  img[0].theta			= -6 * PI/180.;	  // 0: face-on, pi/2: edge-on
  img[0].distance		= 56*PC;	      // source distance in m
  img[0].source_vel		= 13.4e3;         // source velocity in m/s
  img[0].unit			= 1;		      // 0:Kelvin 1:Jansky/pixel 2:SI 3:Lsun/pixel 4:tau
  img[0].filename		= "TW_CO21_10000p_5000s_7i.fits";  // Output filename
...
velocity(double x, double y, double z, double *vel){
  ...
  //orbital velocity
  v=sqrt((0.74*1.989e30*6.67e-11)/rToUse); //mass=2.3Msol
